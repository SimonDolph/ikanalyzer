...
public class IKQueryExpressionParser {
    ...
    private Query toBooleanQuery(Element op) {
        ...
        BooleanQuery.Builder resultQuery = new BooleanQuery.Builder().setDisableCoord(false);
        ...
        if ('&' == op.type) {
            ...
            if (q2 != null) {
                if (q2 instanceof BooleanQuery) {
                    ...
                    if (clauses != null && clauses.size() > 0 && clauses.get(0).getOccur() == Occur.MUST) {
                        for (BooleanClause c : clauses) {
                            resultQuery.add(c);
                        }
                    } else {
            ...
            if (q1 != null) {
                if (q1 instanceof BooleanQuery) {
                    List<BooleanClause> clauses = ((BooleanQuery) q1).clauses();
                    if (clauses != null && clauses.size() > 0 && clauses.get(0).getOccur() == Occur.SHOULD) {
                        for (BooleanClause c : clauses) {
                            resultQuery.add(c);
                        }
                    } else {
